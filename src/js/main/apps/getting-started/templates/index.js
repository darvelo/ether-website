...
<div class="container">
    ...
    <section class="tutorial">
        ...
        <p>The TwitterRoute is rendered when the user navigates to the URL <code>'/app/{twitter_username=\\\\w+}/{tweet_id=\\\\d+}'</code>. Since expectedParams() lists both params, the route will have both <code>twitter_username</code> and <code>tweet_id</code> available in the <code>params</code> argument of its prerender() and render() methods. Also, any query params in the URL will be in the <code>queryParams</code> argument. The <code>diffs</code> argument is the difference between the params and queryParams parsed from the current URL vs. the URL that last rendered the class. If there was no difference, the value will be null.</p>
        ...
        <p>If the user then navigates somewhere else then back to the same URL, the params and queryParams arguments will be the same, but the diffs argument will be null since neither the params nor queryParams changed compared to the URL when the TwitterRoute was last rendered.</p>
