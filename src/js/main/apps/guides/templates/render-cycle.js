...
<p>Each subclass you make of App and Route has three render cycle methods available to it: <code>prerender()</code>, <code>render()</code>, and <code>deactivate()</code>. When each method is called for which subclass depends on how you've mounted your Apps and Routes using the RootApp/App <code>mount()</code> and <code>mountConditionals()</code> methods.</p>
<p>Let's begin by defining some Apps and Routes:</p>
<pre><code class="hljs js">${mountExample()}</code></pre>

<p>Note that we've omitted showing method definitions for the apps, and Route definitions entirely. We're just focusing on the classes themselves and how they relate, to learn about how their render cycle methods will be called and in whatorder.</p>

<p>When navigating to a URL, Ether inspects the mount hierarchy and determines the navigation path through that hierarchy for the URL. It does this by matching the strings you return from <code>mount()</code> and matching them against the URL, descending to deeper mount nodes and matching against their strings until a Route subclass is found that matches against the final part of the URL.</p>
...
<p>RootApp/App <code>prerender()/render()</code> calls are always made before those methods are called on any of their mounts or conditional mounts. RootApp/App <code>deactivate()</code> calls are always made after the same method is called on <strong>all</strong> currently-rendered mounts and conditional mounts.</p>
<p>The reasoning for this is that an App's mounts may depend on the App itself having been rendered before they can be rendered. Likewise, deactivating an App only seems logical once all of its dependents are deactivated.</p>
<p>Ether sets state hooks on a mount instance before and after render cycle functions are executed. Check out the section on the <code>this.state</code> property in the <a href="${ctx.hrefs.sharedMethods}">Shared Methods and Properties docs </a> for more info.</p>

<h2>Scenario 0</h2>
<pre>${scenario0()}</pre>
<p>This scenario occurs only on the first call to navigate, either explicitly through the <a href="${ctx.hrefs.sharedMethods}">shared method</a> <code>navigate()</code> or through any of the event handlers set on the <a href="${ctx.hrefs.rootAppDocs}">RootApp options</a>.</p>
...
<p>Notice how stages of the Render Phase match the stages of the Prerender Phase exactly. This is part of what makes the render cycle predictable.</p>
